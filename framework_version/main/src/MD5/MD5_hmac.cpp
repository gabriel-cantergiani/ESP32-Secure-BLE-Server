...
char* MD5_hmac::make_digest(const unsigned char *digest, int len) /* {{{ */
{	
	char * md5array = (char*) malloc(sizeof(char)*(len));
 ...
 */
const void *MD5_hmac::body(void *ctxBuf, const void *data, size_t size)
{
...
void MD5_hmac::MD5Init(void *ctxBuf)
{
	MD5_CTX *ctx = (MD5_CTX*)ctxBuf;
	ctx->a = 0x67452301;
	ctx->b = 0xefcdab89;
...
void MD5_hmac::MD5Update(void *ctxBuf, const void *data, size_t size)
{
	MD5_CTX *ctx = (MD5_CTX*)ctxBuf;
...
void MD5_hmac::MD5Final(unsigned char *result, void *ctxBuf)
{
	MD5_CTX *ctx = (MD5_CTX*)ctxBuf;
	MD5_u32plus used, free;
...
char* MD5_hmac::hmac_md5(const void *text, int text_len,void *key, int key_len){
